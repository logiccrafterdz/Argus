...
void ExecuteTrade(ENUM_ORDER_TYPE type, double sl_ref)
{
   ...
   if(type == ORDER_TYPE_BUY) {
      ...
      CArgusCore::ExecuteTradeWithRetry(trade, _Symbol, ORDER_TYPE_BUY, lot, ask, sl, tp, "Donchian Breakout Long");
   }
   else {
      double sl = CArgusCore::NormalizePrice(_Symbol, sl_ref + (2 * _Point), tick_sz);
      ...
      double lot = CArgusCore::CalculateLotSize(_Symbol, RiskPercent, risk_dist, vol_precision);
      CArgusCore::ExecuteTradeWithRetry(trade, _Symbol, ORDER_TYPE_SELL, lot, bid, sl, tp, "Donchian Breakout Short");
   }
}
...
StrategyManifest GetManifest()
{
   ...
   m.target_style = "Trailing Opposite Band";
   return m;
}
