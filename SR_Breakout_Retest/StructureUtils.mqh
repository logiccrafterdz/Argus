...
public:
   ...
   // Get the price of the most recent Swing High within range
   static double GetRecentSwingHigh(int lookback, int radius)
   {
      for(int i = radius; i < lookback; i++) {
         if(IsSwingHigh(i, radius)) return iHigh(_Symbol, _Period, i);
      }
      return 0;
   }

   // Get the price of the most recent Swing Low within range
   static double GetRecentSwingLow(int lookback, int radius)
   {
      for(int i = radius; i < lookback; i++) {
         if(IsSwingLow(i, radius)) return iLow(_Symbol, _Period, i);
      }
      return 0;
   }
};
