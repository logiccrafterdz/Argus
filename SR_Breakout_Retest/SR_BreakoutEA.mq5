...
//+------------------------------------------------------------------+
//| Phase 1: Identify level and detect breakout                      |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Phase 2 & 3: Monitor Retest and Confirmation                    |
//+------------------------------------------------------------------+
void MonitorRetest()
{
   break_bar_index++;
   
   // 1. Validity Check (Max Bars)
   ...
   // 2. Distance Filter
   double dist_pips = MathAbs(last_close - active_level) / PipsToPriceDelta(1);
   if(dist_pips > MaxBreakDistancePips) {
      PrintFormat("Price moved too far (%.1f pips). Resetting.", dist_pips);
      ResetState();
      return;
   }

   // 3. Logic for Buy Retest & Confirmation
   if(intended_signal == SIGNAL_BUY) {
      bool touch = (last_low <= active_level);
      bool confirm = (last_close > prev_high); // Close above previous bar after touch
   ...
   // 4. Logic for Sell Retest & Confirmation
   else if(intended_signal == SIGNAL_SELL) {
      bool touch = (last_high >= active_level);
      bool confirm = (last_close < prev_low); // Close below previous bar after touch
...
void OpenPosition(ENUM_ORDER_TYPE type)
{
   ...
   if(type == ORDER_TYPE_BUY) {
      ...
      ExecuteTrade(ORDER_TYPE_BUY, lot, ask, sl, tp, "SR Breakout Buy");
   }
   else {
      ...
      double lot = CalculateLotSize(risk_dist);
      ExecuteTrade(ORDER_TYPE_SELL, lot, bid, sl, tp, "SR Breakout Sell");
   }
}
...
void ResetState() { current_state = STATE_IDLE; intended_signal = SIGNAL_NONE; active_level = 0; break_bar_index = 0; }
