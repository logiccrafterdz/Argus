...
def run_test(strat_class, params_overrides, label=None):
    ...
    regimes = {}
    for sym in data:
        tf_reg = 'H4' if 'H4' in data[sym] else ('H1' if 'H1' in data[sym] else None)
        if tf_reg:
            h = data[sym][tf_reg]
    ...
    return {
        ...
        'trades': trades,
        'pf': round(pf, 4),
        'return': round(port.get('total_return', 0), 2),
        'dd': round(port.get('max_drawdown', 0), 2),
        'wr': round(wr, 1),
        'net': round(port.get('net_profit', 0), 2),
    }
        ...
        if fname.endswith('.py') and not fname.startswith('_'):
            ...
            try:
                ...
                if hasattr(mod, args.strategy):
                    ...
                    result = run_test(cls, params, label=args.label)
