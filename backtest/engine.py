...
from signal_types import StrategyType, STRATEGY_TYPE_PARAMS
...
class BacktestEngine:
    def __init__(self, initial_balance=100000.0, config=None):
        ...
        self.tick_size_map = {
            ...
            'US30': tick_cfg.get('US30', 0.1),
            'NAS100': tick_cfg.get('NAS100', 0.01),
            'EURJPY': tick_cfg.get('EURJPY', 0.001),
            'GBPJPY': tick_cfg.get('GBPJPY', 0.001),
        }
        self.default_tick_size = self.config.get('default_tick_size', 0.00001)

        pip_cfg = self.config.get('pip_size', {})
        self.pip_size_map = {}
        self.default_pip_size = self.config.get('default_pip_size', 0.0001)

        tv_cfg = self.config.get('tick_value', {})
        self.tick_value_map = {}
        self.default_tick_value = self.config.get('default_tick_value', 1.0)

        cm_cfg = self.config.get('commission', {})
        self.commission_map = cm_cfg
        self.default_commission = self.config.get('default_commission', 7.0)

        self.usd_multiplier_map = {}
        self.default_usd_multiplier = self.config.get('default_usd_multiplier', 1.0)

        swap_cfg = self.config.get('swap', {})
        self.swap_map = swap_cfg
        self.default_swap = self.config.get('default_swap', (-0.5, -0.5))

        self.slippage_min = self.config.get('slippage_min', 0)
        self.slippage_max = self.config.get('slippage_max', 0.3)

        self.atr_map = {}

        self.strategy_trades = {}
        self.kelly_window = 50
        self.kelly_enabled = self.config.get('kelly', {}).get('enabled', False)
    ...
    def get_usd_multiplier(self, symbol):
        if symbol in self.usd_multiplier_map:
            return self.usd_multiplier_map[symbol]
        if symbol.endswith('JPY'):
            return 1.0 / 100.0
        if symbol == 'XAUUSD' or symbol == 'US30' or symbol == 'NAS100':
            return 1.0
        if 'USD' in symbol:
            if symbol.startswith('USD'):
                if hasattr(self, 'current_prices') and 'USDCHF' in self.current_prices:
    ...
    def contract_size(self, symbol):
        return self.contract_size_map.get(symbol, self.default_contract_size)

    def tick_size(self, symbol):
        return self.tick_size_map.get(symbol, self.default_tick_size)

    def pip_size(self, symbol):
        return self.pip_size_map.get(symbol, self.default_pip_size)

    def tick_value(self, symbol):
        return self.tick_value_map.get(symbol, self.default_tick_value)
    ...
    def get_kelly_multiplier(self, strategy_name):
        ...
        base_risk = self.kelly_base_risk / 100.0
    ...
    def compute_sl_tp(self, strategy_type, order_type, entry_price, atr_value, strategy_tp=None):
        params = STRATEGY_TYPE_PARAMS.get(strategy_type, STRATEGY_TYPE_PARAMS['trend_momentum'])
        sl_atr = params['sl_atr']
        if strategy_type == 'trend_momentum':
            if order_type == 'BUY':
                sl = entry_price - sl_atr * atr_value
                tp = strategy_tp if strategy_tp is not None else entry_price + params['tp_atr'] * atr_value
            else:
                sl = entry_price + sl_atr * atr_value
                tp = strategy_tp if strategy_tp is not None else entry_price - params['tp_atr'] * atr_value
        else:
            tp_atr = params.get('tp_atr', sl_atr * 2)
            if order_type == 'BUY':
                sl = entry_price - sl_atr * atr_value
    ...
    def execute_trade(self, symbol, strategy_name, strategy_type, order_type, lot_size, entry_price, atr_value, comment, strategy_tp=None, override_sl=None, override_tp=None):
        ...
        strat_type_val = strategy_type.value if hasattr(strategy_type, 'value') else strategy_type
        pos = {
            'ticket': self.ticket_counter,
            'symbol': symbol,
            'strategy': strategy_name,
            'strategy_type': strat_type_val,
            'type': order_type,
    ...
    def update(self, current_time, current_prices, current_highs, current_lows):
        ...
        for pos in list(self.open_positions):
            ...
            pos_type = pos.get('strategy_type', 'trend_momentum')
            risk_distance = abs(pos['entry_price'] - pos['sl'])

            # Track highest/lowest prices and unrealized profit
            if pos['type'] == 'BUY':
            ...
            if pos_type == 'mean_reversion':
                if not pos['breakeven_triggered'] and risk_distance > 0:
                    ...
                    if pos['type'] == 'BUY' and high >= pos['entry_price'] + risk_distance:
                        pos['sl'] = pos['entry_price'] + spread
                        pos['breakeven_triggered'] = True
                    elif pos['type'] == 'SELL' and low <= pos['entry_price'] - risk_distance:
                        pos['sl'] = pos['entry_price'] - spread
                        pos['breakeven_triggered'] = True
                if not pos['partial_closed'] and risk_distance > 0:
                    if pos['type'] == 'BUY' and high >= pos['entry_price'] + risk_distance * 1.5:
                        half = pos['remaining_lots'] * 0.5
                        self.close_position_partial(pos, pos['entry_price'] + risk_distance * 1.5, current_time, "Partial TP 1.5R", half)
                        pos['partial_closed'] = True
                    elif pos['type'] == 'SELL' and low <= pos['entry_price'] - risk_distance * 1.5:
                        half = pos['remaining_lots'] * 0.5
                        self.close_position_partial(pos, pos['entry_price'] - risk_distance * 1.5, current_time, "Partial TP 1.5R", half)
                        pos['partial_closed'] = True
            elif pos_type == 'stop_hunt':
                if not pos['breakeven_triggered'] and risk_distance > 0:
                    spread = self.spread_map.get(symbol, self.default_spread)
                    if pos['type'] == 'BUY' and high >= pos['entry_price'] + risk_distance * 0.5:
                        pos['sl'] = pos['entry_price'] + spread
                        pos['breakeven_triggered'] = True
                    elif pos['type'] == 'SELL' and low <= pos['entry_price'] - risk_distance * 0.5:
                        pos['sl'] = pos['entry_price'] - spread
                        pos['breakeven_triggered'] = True
                if not pos['partial_closed'] and risk_distance > 0:
                    if pos['type'] == 'BUY' and high >= pos['entry_price'] + risk_distance * 2.0:
                        half = pos['remaining_lots'] * 0.5
                        self.close_position_partial(pos, pos['entry_price'] + risk_distance * 2.0, current_time, "Partial TP 2.0R", half)
                        pos['partial_closed'] = True
                    elif pos['type'] == 'SELL' and low <= pos['entry_price'] - risk_distance * 2.0:
                        half = pos['remaining_lots'] * 0.5
                        self.close_position_partial(pos, pos['entry_price'] - risk_distance * 2.0, current_time, "Partial TP 2.0R", half)
                        pos['partial_closed'] = True
            ...
            if pos['type'] == 'BUY':
                hit_sl = low <= pos['sl']
                hit_tp = pos['tp'] is not None and high >= pos['tp']
                ...
                hit_sl = high >= pos['sl']
                hit_tp = pos['tp'] is not None and low <= pos['tp']
                if hit_sl and hit_tp:
